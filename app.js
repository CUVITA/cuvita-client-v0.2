...
App({
    ...
    onLaunch: function() {
        wx.showLoading({
            title: this.GlobalLocalePackage.loading[
                Store.getState().global.locale
            ],
            mask: true
        });
        Store.dispatch(GlobalActions.setSystemInfo(wx.getSystemInfoSync()));
        Store.dispatch(
            GlobalActions.setLocale(wx.getStorageSync("locale") || 0)
        );
        Store.dispatch(
            GlobalActions.setMember(wx.getStorageSync("member") || null)
        );
        promisfy
            .login()
