...
Page({
    ...
    onLoad: function(options) {
        ...
        this.unsubscribe = Store.subscribe(() => {
            mapStateToPage(Store, this, {
                locale: "global.locale",
                region: "global.region"
            });
        });
        options.fallback !== "region"
            ? promisfy
                  .getLocation()
                  .then(({ latitude, longitude }) => {
                      return promisfy.fetch("/region/nearest", {
                          lat: latitude,
                          long: longitude
                      });
                  })
                  .then(region => {
                      region
                          ...
                          : wx.showModal({
                                title:
                                    localePackage.modal.noservice.title[
                                        this.data.locale
                                    ],
                                content:
                                    localePackage.modal.noservice.content[
                                        this.data.locale
                                    ],
                                showCancel: false,
    ...
    onScroll: function({ detail: { scrollTop } }) {
        if (
            this.data.scrollTop > -this.data.systemInfo.screenWidth * 1.2 &&
            scrollTop <= -this.data.systemInfo.screenWidth * 1.2
        )
            this.feedback();
        this.setData({
            scrollTop
